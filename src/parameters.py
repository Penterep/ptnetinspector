...
def parameter_control(interface, json_output, del_tmp, type, more_detail, less_detail, check_addresses, ipv4, ipv6, duration_passive, duration_aggressive, prefix,
    ...
    If there is error, it returns errors print
    '''
    list_error = []
    list_warning = []
    ...
    if interface is not None:
        ...
        if interface in valid_interface:
            ...
            err = "Invalid inserted interface: " + interface + ". Program exits!"
            list_error.append(err)

            if not json_output or more_detail:
                ptprinthelper.ptprint("Invalid inserted interface: " + interface + ". Program exits!", "ERROR")
            if json_output:
    ...
    if more_detail and less_detail:
        ...
        if not json_output or more_detail:
            ptprinthelper.ptprint("Showing full detail and less detail can not be set at the same time. Program exits!", "ERROR")
        if json_output:
            print(ptjsonlib_object.end_error("Showing full detail and less detail can not be set at the same time. Program exits!", ptjsonlib_object))
        sys.exit(1)
    ...
    if type == ["p"] or ("p" in type and "802.1x" in type):
        if duration_passive is None:
            duration_passive = 30
            war = "Missing passive duration, so the default value is chosen: " + str(duration_passive) + " s"
            list_warning.append(war)

        if duration_passive is not None:
            if not is_non_negative_float(duration_passive):
        ...
        if duration_aggressive is not None:
            err = "Aggressive duration is not applied in this mode. Program exits!"
            list_error.append(err)

        if prefix is not None:
            err = "Network prefix is not applied in this mode. Program exits!"
            list_error.append(err)
        ...
        if smac is not None:
            err = "Source MAC is not applied in this mode. Program exits!"
            list_error.append(err)

        if sip is not None:
            err = "Source IP is not applied in this mode. Program exits!"
            list_error.append(err)
            
        if rpref is not None:
            err = "Preference flag in RA is not applied in this mode. Program exits!"
            list_error.append(err)

        if period is not None:
            err = "Period (RA sending rate) is not applied in this mode. Program exits!"
            list_error.append(err)

        if chl is not None:
            err = "Current hop limit is not applied in this mode. Program exits!"
            list_error.append(err)

        if mtu is not None:
            err = "MTU is not applied in this mode. Program exits!"
            list_error.append(err)

        if dns is not None:
            err = "DNS address is not applied in this mode. Program exits!"
            list_error.append(err)
        ...
        if not Interface(interface).check_available_ipv6():
            err = "No available IP on the interface: " + interface + ". Program exits!"
            list_error.append(err)
    ...
    if type == ["a+"] or ("a+" in type and ("802.1x" in type or "a" in type)):
        ...
        if duration_aggressive is None:
            ...
            war = "Missing aggressive duration, so the default value is chosen: " + str(duration_aggressive) + " s"
            list_warning.append(war)

        if duration_aggressive is not None:
            if not is_non_negative_float(duration_aggressive):
        ...
        if smac is None:
            ...
            war = "Missing source MAC, so scanner's MAC is resolved from interface: " + smac
            list_warning.append(war)

        elif smac is not None:
            if not is_valid_mac(smac):
                err = "Invalid inserted MAC address. Program exits!"
                list_error.append(err)

        # Checking the IPv6 address
        if sip is not None:
            if not is_valid_ipv6(sip):
        ...
        if sip is None:
            if Interface(interface).check_available_ipv6():
                ...
                for sip in sip_list:
                    sip.split('%', 1)[0]
                    sip_list_new.append(sip)          
                war = "Missing source IP, so scanner's IP is resolved from interface: " + str(sip_list_new)
                list_warning.append(war)
                sip = sip_list_new
            else:
                err = "No available IP on the interface: " + interface + ". Program exits!"
        ...
        if is_non_negative_float(duration_aggressive):
            if period is None:
                ...
                war = "Missing period (RA sending rate), so it is set to: 1 RA /" + str(period) + " s"
                list_warning.append(war)
            ...
            if period is not None:
                ...
                if is_non_negative_float(period):
                    if float(period) > float(duration_aggressive):
        ...
        # Checking the current hop limit
        if chl is None:
            chl = 0
    ...
    if not less_detail:
        ...
        # Control parameter for deleting tmp files
        if not del_tmp:
