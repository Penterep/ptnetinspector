...
class IGMPv3(Node):
    ...
    def save(self):
        with open('src/tmp/IGMPv3.csv', 'a+', newline='') as csvfile:
            csvfile.seek(0)
            for row in csv.DictReader(csvfile):
                if row and row['MAC'] == self.mac and row['IP'] == self.ip and row['protocol'] == self.protocol and row[
                    'rtype'] == self.rtype and row['mulip'] == self.mulip and row['sources'] == self.sources:
                    return
